...
void zzn2_mirvar(_MIPD_ zzn2 *w){
    w->a = mirvar(_MIPP_ 0);
    w->b = mirvar(_MIPP_ 0);
}

void zzn2_kill(_MIPD_ zzn2*w){
    mirkill(w->a);
    mirkill(w->b);
}
...
void zzn2_pow(_MIPD_ zzn2 *x, big e, zzn2*w){
    int i,j,nb,n,nbw,nzs;
    big ONE = mirvar(_MIPP_ 1);
    big ZERO = mirvar(_MIPP_ 0);
    
    zzn2 u,u2,t[16];
    zzn2_mirvar(_MIPP_ &u);
    zzn2_mirvar(_MIPP_ &u2);
    for (i=0;i<16;i++) zzn2_mirvar(_MIPP_ &(t[i]));
    ...
    if (zzn2_iszero(x)) {
        ...
        zzn2_kill(_MIPP_ &u);
        zzn2_kill(_MIPP_ &u2);
        for (i=0;i<16;i++) zzn2_kill(_MIPP_ &(t[i]));
        return;
    }
    ...
    zzn2_copy(x, &u);
    zzn2_mul(_MIPP_ &u, &u, &u2);
    zzn2_copy(&u, &(t[0]));
    
    for (i=1;i<16;i++) zzn2_mul(_MIPP_ &u2, &(t[i-1]), &(t[i]));
    
    nb = logb2(_MIPP_ e);
    if (nb>1) for (i=nb-2;i>=0;)
    {
        n = mr_window(_MIPP_ e, i, &nbw, &nzs, 5);
        for (j=0;j<nbw;j++) zzn2_mul(_MIPP_ &u, &u, &u);
        if (n>0)  zzn2_mul(_MIPP_ &u, &(t[n/2]), &u);
        i-=nbw;
        if (nzs)
        {
            for (j=0;j<nzs;j++) zzn2_mul(_MIPP_ &u, &u, &u);
            i-=nzs;
        }
    }
